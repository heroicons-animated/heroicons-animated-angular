...
@Component({
  selector: 'hi-clipboard-document-check',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    '[attr.aria-label]': "'clipboard-document-check'",
    role: 'img',
  ...
  >
    ...
    />
    <path
      class="clipboarddocumentcheck-check"
      [class.clipboarddocumentcheck-draw]="shouldAnimate()"
      pathLength="1"
  ...
  styles: [
    `
      ...
      /* pathLength [0,1] opacity [0,1] 0.4s easeInOut (matches React PATH_VARIANTS) */
      ...
      .clipboarddocumentcheck-check.clipboarddocumentcheck-draw {
        animation: clipboarddocumentcheck-draw 0.4s ease-in-out forwards;
      }

      @keyframes clipboarddocumentcheck-draw {
        0% {
          stroke-dashoffset: 1;
          opacity: 0;
        }
