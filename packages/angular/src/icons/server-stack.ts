...
@Component({
  selector: "hi-server-stack",
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    "[attr.aria-label]": "'server-stack'",
    role: "img",
  ...
  template: `<svg
    ...
    class="icon-svg"
    [class.serverstack-stretch]="shouldAnimate()"
  >
    <path
  ...
  styles: [
    `
      ...
      .icon-svg.serverstack-stretch {
        animation: serverstack-stretch 0.4s ease-in-out forwards;
      }

      @keyframes serverstack-stretch {
        0%,
        100% {
          transform: scaleY(1);
        }
