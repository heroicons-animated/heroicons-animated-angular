...
@Component({
  selector: 'hi-inbox-stack',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    '[attr.aria-label]': "'inbox-stack'",
    role: 'img',
  ...
  template: `<svg
    ...
    class="icon-svg"
    [class.inboxstack-animate]="shouldAnimate()"
  >
    <path
  ...
  styles: [
    `
      ...
      .icon-svg.inboxstack-animate {
        animation: inboxstack-animate 0.4s ease-in-out;
      }

      @keyframes inboxstack-animate {
        0% {
          transform: scaleY(1);
        }
        50% {
