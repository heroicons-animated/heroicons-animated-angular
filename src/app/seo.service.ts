...
export class SeoService {
  ...
  setIconMeta(icon: IconMeta) {
    ...
    this.setDescription(description);
    this.setKeywords(
      [...icon.keywords, 'animated icon', 'angular icon', `${icon.name} animation`].join(', '),
    );
    this.setRobots('index,follow');
    this.setOpenGraph({
      url: canonical,
    ...
    this.setJsonLd('icon-schema', {
      ...
      codeRepository: 'https://github.com/heroicons-animated/heroicons-animated-angular',
  ...
  private setTwitter(data: { title: string; description: string; image: string; imageAlt: string }) {
